...
class StringTable
{
	...
	std::string ToUtf8(uint16_t _count)
	{
		if (_count >= count)
		{
		...
		const std::vector<std::byte>& data = strings[_count];

		std::vector<uint8_t> byteData;
		for (const auto& byte : data)
		{
			byteData.push_back(static_cast<uint8_t>(byte));  // ½« std::byte ×ª»»Îª uint8_t
		}

		RawString rawStr = RawString::FromBytes(byteData);

		return rawStr.ToUTF8String();
	}
};